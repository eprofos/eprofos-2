        ...
        if ($file->isFile() && $file->getExtension() === 'php') {
            ...
            if (preg_match('/name:\s*[\'"]([a-zA-Z0-9_]+_)[\'"]/', $content, $matches)) {
                ...
                foreach ($lines as $line) {
                    if (str_contains($line, '#[Route') && str_contains($line, 'name:') && str_contains($line, $matches[1])) {
                        $controllers[] = [
                            'file' => $filePath,
                            'content' => $content,
                            'prefix' => $matches[1],
...
function processController(array $controllerInfo, bool $dryRun = false): bool
{
    ...
    foreach ($lines as $lineNum => $line) {
        ...
        if (str_contains($line, '#[Route') && str_contains($line, 'name:') && str_contains($line, $prefix)) {
