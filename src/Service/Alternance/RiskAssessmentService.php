...
class RiskAssessmentService
{
    ...
    public function generateRiskReport(\DateTimeInterface $startDate, \DateTimeInterface $endDate): array
    {
        ...
        return [
            ...
            'students_needing_immediate_attention' => array_filter($riskData, function($student) {
                return $student['risk_level'] >= self::HIGH_RISK_THRESHOLD;
            }),
    ...
    private function analyzeRiskFactors(Student $student, ProgressAssessment $latestProgress, StudentProgress $studentProgress): array
    {
        ...
        if (!empty($difficulties)) {
            $severeDifficulties = array_filter($difficulties, function($difficulty) {
                return ($difficulty['severity'] ?? 0) >= 4;
            });
        ...
        if (!empty($supportNeeded)) {
            $urgentSupport = array_filter($supportNeeded, function($support) {
                return ($support['urgency'] ?? 0) >= 4;
            });
        ...
        if ($studentProgress->getAlternanceContract()) {
            ...
            if (!empty($missionProgress)) {
                $completedMissions = array_filter($missionProgress, function($mission) {
    ...
    private function calculateRiskLevel(array $riskFactors): int
    {
        ...
        if ($averageWeight <= 1.2) return 1;
        if ($averageWeight <= 1.7) return 2;
        if ($averageWeight <= 2.3) return 3;
        if ($averageWeight <= 2.8) return 4;
        return 5;
    }
    ...
    {
        $highRiskCount = count(array_filter($riskData, function($student) {
            return $student['risk_level'] >= self::HIGH_RISK_THRESHOLD;
        }));
    ...
    private function calculateRiskTrend(array $recentAssessments): float
    {
        ...
        $riskLevels = array_map(function($assessment) {
            return $assessment->getRiskLevel();
        }, array_reverse($recentAssessments)); // Reverse for chronological order
    ...
    {
        if ($predictedRisk >= 4) return 1; // 1 week
        if ($predictedRisk >= 3) return 2; // 2 weeks
        return 4; // 1 month
    }
