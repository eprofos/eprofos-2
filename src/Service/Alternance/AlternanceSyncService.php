    ...
    public function generateSyncReport(Student $student): array
    {
        ...
        $report = [
            ...
            'statistics' => [
                ...
                'confirmed_weeks' => count(array_filter($calendar, static fn ($c) => $c->isConfirmed())),
                'center_weeks' => count(array_filter($calendar, static fn ($c) => $c->getLocation() === 'center')),
                'company_weeks' => count(array_filter($calendar, static fn ($c) => $c->getLocation() === 'company')),
                'weeks_with_activities' => count(array_filter($calendar, static fn ($c) => !empty($c->getAllActivities()))),
                'weeks_with_notes' => count(array_filter($calendar, static fn ($c) => !empty($c->getNotes()))),
            ],
            'export_formats' => [
                'ical_available' => true,
                'csv_available' => true,
