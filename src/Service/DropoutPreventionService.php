...
class DropoutPreventionService
{
    ...
    public function detectAtRiskStudents(): array
    {
        ...
        usort($atRiskStudents, fn($a, $b) => $b['riskScore'] <=> $a['riskScore']);
    ...
    public function generateRetentionReport(): array
    {
        try {
            ...
            $atRiskCount = count(array_filter($allProgress, fn($p) => $p->isAtRiskOfDropout()));
            $completedCount = count(array_filter($allProgress, fn($p) => $p->getCompletedAt() !== null));
    ...
    public function exportRetentionData(string $format = 'array'): array
    {
        ...
        $exportData = [
            ...
            'at_risk_students' => array_map(function($item) {
                return [
