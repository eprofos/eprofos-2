    ...
    public function index(): Response
    {
        try {
            ...
            $this->logger->debug('Accessible formations retrieved', [
                ...
                'formation_ids' => array_map(fn($f) => $f->getId(), $accessibleFormations)
            ]);
            ...
            $completedEnrollments = array_filter($enrollments, fn($e) => $e->isCompleted());
            ...
            foreach ($enrollments as $index => $enrollment) {
                ...
                try {
                    ...
                    if ($progress) {
                        ...
                        $completedModules = array_filter($moduleProgress, fn($m) => $m['completed'] ?? false);
            ...
            foreach ($enrollments as $index => $enrollment) {
                ...
                try {
                    ...
                    if ($progress) {
                        ...
                        $completedSessions += count(array_filter($courseProgress, fn($c) => $c['completed'] ?? false));
                        ...
                        $this->logger->debug('Progress data processed for study time stats', [
                            ...
                            'completed_sessions_for_enrollment' => count(array_filter($courseProgress, fn($c) => $c['completed'] ?? false)),
