...
class StudentProgressFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    private function setProgressMetrics(StudentProgress $progress, string $profileType, int $daysRunning, $faker): void
    {
        ...
        $expectedLogins = intval($daysRunning * $loginFrequency);
        $loginCount = max(1, $faker->numberBetween(intval($expectedLogins * 0.7), intval($expectedLogins * 1.3)));
    ...
    private function setCurrentProgress(
        ...
        $formationModules = array_filter($modules, fn($module) => $module->getFormation() === $formation);
        $formationChapters = array_filter($chapters, function($chapter) use ($formationModules) {
            return in_array($chapter->getModule(), $formationModules);
        });
        ...
        if (!empty($formationModules) && $completion < 100) {
            ...
            $moduleIndex = intval(($completion / 100) * count($formationModules));
            ...
            if ($currentModule) {
                ...
                $moduleChapters = array_filter($formationChapters, fn($chapter) => $chapter->getModule() === $currentModule);
